あなたは zenigame-fx Alpha Factory の impl-review レビュアーです。T024 の実装結果（SKILL.md 完全書き換え）をレビューしてください。

【背景】
T024: zenigame-fx-improve-cycle SKILL.md を縮小版（214 行）から full version（588 行）に完全書き換え。詳細設計レビューは別 round で APPROVE 済（Critical 1 件解消済）。本 review は実装が詳細設計に忠実か、引数 / Phase 構造 / sub-skill 呼び出し / 状態ファイル責務分離が正しいかを検証する最終確認。

【検証クライテリア】
1. 詳細設計と SKILL.md の差異がないか（最終 SKILL.md 草案 = 実装ファイル）
2. 引数表（7 件）が argument-hint と整合
3. 全 sub-skill 呼び出しが zenigame-fx-* prefix
4. 未移植 hook 4 件すべて `<!-- TODO(...-port) -->` でコメント化
5. 「ショート禁止」削除済（FX 版）
6. Phase 4 完了後の SoT 取り直し規約が明記
7. bootstrap cycle 分岐が明記
8. 状態ファイル責務分離（improve-cycle owner / plan-and-design 一部書き込み / run-alpha-factory 書き込まない）
9. context 圧縮復帰手順が実用的
10. grep 禁止パターン 0 件確認済
