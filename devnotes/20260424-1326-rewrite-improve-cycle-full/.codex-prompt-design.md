あなたは zenigame-fx Alpha Factory の design レビュアーです。以下の詳細設計（最終 SKILL.md 草案）を C9 falsification-first 観点でレビューしてください。

【背景】
T024: zenigame-fx-improve-cycle SKILL.md を縮小版（214 行）から full version へ完全書き換え。zenigame 版（archive 済み 476 行）を base に T020-T023 で port 済の sub-skill 群を活用。Phase 2 architecture（Stage A/B/C / GenomeArchive / cross-pair / swim-lane / 32 primitive）対応。

概念設計は別 round で APPROVE 相当（Critical 1 件 + Warning 3 件はすべて反映済）。
...
- 取引回数を削減して見かけ成績を上げる（禁止事項 6）
- 評価期間延長で過学習を隠す（禁止事項 1）
- live_criteria を緩和してステージを飛ばす（禁止事項 4）
- 見た目の数値だけ良くする（禁止事項 2）
- オーバーナイト保有前提の設計（禁止事項 7）

【レビュー観点】
1. SKILL.md 草案の正確性（誤記 / 矛盾 / 引数誤り）
2. 引数 → sub-skill 引数への passthrough 整合性
3. 状態ファイル責務分離が SKILL.md 文面と一致するか
4. Phase 4 完了後の SoT 取り直し規約の手順網羅性（Phase 5 に渡す run_number が確実に SoT になっているか）
5. Emergency Fix モード（producer 不在 fail-soft）の挙動明確性
6. 未移植 hook 6 件すべてコメント化されており、削除されていないか
7. grep 禁止パターン（zenigame-* / docs/alpha-factory / src/trading）が含まれていないか
8. 繰り返しモードの自律性（ユーザー問い合わせなし）の徹底
9. context 圧縮復帰手順の実用性
10. 使用例の網羅性
11. 全体構造図と Phase 詳細の整合性
...
```
1 件もなければ「初回サイクル」として Phase 1 を skip し Phase 2 から開始（plan-and-design に渡す `run_id` は null、analysis-* も空でフォールバック）。
