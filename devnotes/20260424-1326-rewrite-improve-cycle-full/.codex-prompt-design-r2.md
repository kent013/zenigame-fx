あなたは zenigame-fx Alpha Factory の design レビュアーです。前回ラウンドで以下の Critical 1 件指摘を受け、詳細設計を修正しました。再レビューしてください。

【前回の Critical】
H1: archive 0 件初回サイクルで Phase 2 へ進める設計だが、plan-and-design は analysis を入力前提として停止する → 設計衝突。

【最小変更】
Step 0-2 の bootstrap 分岐を追加:
- archive Parquet 0 件時は **Phase 1 / 2 / 3 を全 skip** し、Phase 4 → Phase 5 で初回 SoT Run を生成（bootstrap cycle）
- 次サイクル先頭で `run_id` を最新 SoT に設定して通常 Phase 1→2→3 フローへ復帰
- run-report は analysis-dir 内に analysis-*.md がなくても warning ログのみで部分生成する仕様（run-report SKILL.md 準拠）
