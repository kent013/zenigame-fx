...
【監査・レビュー時の必須チェック】
C3: Collider bias を report する場合、Conditioning set を必ず明記。
C4: Claim / verdict を出す前に前提を bullet でリスト化。
C6: Fact / Interpretation を分離。
C8: INCONCLUSIVE を第一級 verdict に。
C9: 反証を先に探せ（confirmation bias を避けよ）。

---

# 概念設計レビュー（Round 4）: factor-shadow-plane (FSP)

## 修正内容（Round 3 指摘への対応）

- [Critical] DoD D4 を `D4a(GENOMES_SCHEMA) / D4b(_create_row_template) / D4c(collect_stage_*) / D4d(flush)` に分解
- [Warning] H1 の分母定義を `eligible_rows = bars >= window_days AND factor_data_available` に固定
- [Warning] H3 に長期 INCONCLUSIVE ルール（5 RUN セット連続で検証設計を再定義）を追加
- [Warning] `skipped_conditioning_mismatch` を新 enum 値として追加し、dispatch matrix に 6行目を追加
- 5b の件数不一致時の mode を `skipped_conditioning_mismatch` に明示化
...
重点確認項目:
1. Round 3 の Critical（D4 粒度不足）が解消されているか
2. Warning 3件（H1 分母、H3 長期 INCONCLUSIVE、skipped_conditioning_mismatch）が解消されているか
3. dispatch matrix が 6行（enum 6種）と対応しているか
4. 修正によって新たな矛盾が生じていないか
5. APPROVED 可能かどうかの最終判断
...
## 概念設計ドキュメント（レビュー対象: Round 4 版）

# 概念設計: factor-shadow-plane (FSP) — single instrument 専用 diagnostic layer (Round 4)
...
- `fsp_runtime_mode` ∈ {"active", "skipped_no_factor_data", "skipped_disabled", "skipped_multi_pair_run", "skipped_window_too_short", "skipped_conditioning_mismatch"}
  - `skipped_conditioning_mismatch`: conditioning set の件数不一致 (archive row 数 != FSP 対象個体数) が検出された場合 (Round 3 [Warning] 対応)
...
- **H1 (基盤)**: Run 10 の archive で `fsp_runtime_mode="active"` の row 比率 > 90%
  - 分母定義: `eligible_rows = [rows where bars >= window_days AND factor_data_available]` (Round 3 [Warning] 対応: データ長不足が設計良否判定に混入しないよう分母を固定)
  - 単純 RUN 全体比率での測定は不可 (`skipped_window_too_short` 行が H1 を偽化しうるため)
...
5b. **conditioning set 件数整合チェック** (Round 2 [Warning] 対応):
   ...
   - 不一致の場合は `runtime_mode="skipped_conditioning_mismatch"` に倒して警告ログを出力 (後段への影響なし)
...
| D4b | `_create_row_template` で FSP 新規列の初期値が None / null で設定されている | grep / unit test |
| D4c | `collect_stage_*` で FSP 計算値が archive 行に書き込まれている | unit test |
| D4d | `flush` (Parquet 書き出し) で FSP 列が出力に含まれる | unit test |
| D5 | FSP 計算開始時に conditioning set 件数整合 assert | unit test |
| D6 | `fsp_runtime_mode` の全 enum 値 (active + skipped 6種) にログ出力がある | grep |
| D7 | dispatch matrix の全分岐 (`active`/`skipped_*` 6種) が unit test で網羅されている | pytest coverage |

注: D4a〜D4d は転記漏れが繰り返し発生した「4点セット」(GENOMES_SCHEMA → _create_row_template → collect_stage_* → flush) に対応 (Round 3 [Critical] 対応)。
