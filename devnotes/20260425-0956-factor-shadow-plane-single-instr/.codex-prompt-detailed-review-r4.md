【思考原則】
まず仮説を立てろ。データに真摯に向き合え。機能の名前に立ち返れ。因果ループを切断するな。
...
【ツール使用制限】コマンド実行・ファイル書き込みは一切行わず、提供されたテキストの分析に集中すること。

【監査 discipline】
- C3: conditioning set を明示しない R² 解釈をしない
- C6: 事実と解釈を分離して記載
- C9: Falsification-first — まず「反証を探せ」

---

# 詳細設計レビュー（Round 4）: factor-shadow-plane (FSP)

## タスク

以下の詳細設計 Round 4 をレビューしてください。

**背景**: Round 3 指摘を以下の通り対応しました。

### Round 3 指摘の対応

**Critical 1件 (対応済み)**:
- `TARGET_ROWS` 化で全体重複キー検知が抜ける → 2段構成に変更
  - Step A: `_check_archive_duplicate_keys(archive_df)` を常時実施（既処理行を含む全件チェック）
  - Step B: `_check_key_integrity(target_df, fsp_results)` で TARGET_ROWS × fsp_results の集合一致チェック
  - 呼び出し順序を冪等性フローに明文化

**Warning 2件 (対応済み)**:
- `target_df.empty → "active"` 意味論混線 → `fsp_updater.no_op` ログで `rows_processed=0` を別出力。コメントに「no-op active: rows_processed=0 をログで区別」と明記
- H1 分母に `skipped_conditioning_mismatch` を含む C3 懸念 → `evaluate_key_integrity_failure_rate()` 補助指標を追加。H1 コメントに「整合性障害は H1 で捕捉する」設計意図を明記

## レビュー観点

1. **Round 3 Critical の解消確認**: 2段構成が正しく機能するか
2. **新たな問題**: 修正によって新たな矛盾・欠陥が生まれていないか
3. **全体設計の整合性**: Phase 1 diagnostic-only という使命と各要素が整合するか
4. **4段接続漏れ**: config → FspConfig → AlphaFactoryConfig → run_fsp_updater が完全か
5. **DoD チェックリスト**: D1-D8 が設計本文に対応して残漏れなく完備されているか
...
判定基準:
- APPROVED: Critical 0件、Warning 0-1件（内容が軽微）
...
# 詳細設計: factor-shadow-plane (FSP) — single instrument 専用 diagnostic layer (Round 4)
