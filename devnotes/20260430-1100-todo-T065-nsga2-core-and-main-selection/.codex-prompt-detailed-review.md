...
# T065 詳細設計 — レビュー依頼 (Round 1)

## 役割

zenigame-fx Alpha Factory selection cascade port の **T065 (NSGA-II core + 主選抜 B-pooled) 詳細設計** のレビュアです。 概念設計 Round 4 で APPROVED、 SSOT (§ 11.2) からの転記で詳細実装まで落とし込んだ。 Phase 1 は単体 module + テスト、 Phase 2 (T066/T067/T070/T071 と同時) で 8 箇所同時更新を予定。

## 文脈

- 概念設計: 4 Round で APPROVED (Round 1 [C] 3 + [W] 5 + [S] 3、 Round 2 [C] 1 + [W] 2 + [S] 1、 Round 3 [C] 2 + [W] 1 + [S] 1、 Round 4 [S] 1 全反映)
- 上位文書: synthesis.md § 7.1 / § 7.5 / § 7.7 / § 6.5 (Round 21 改訂後)
- 依存先 (全 APPROVED): T060 Partition+Fold / T061 canonical_metrics (InvariantFlags) / T062 mission_inf_gap (MissionGapResult, constraint_violation) / T063 stage_a_evaluator (a_pass_indices) / T064 stage_bc_evaluator (BCEvaluationResult, pareto_axis_usable, pooled_dd_per_fold_max)
- 主要参考: zenigame `ga/nsga2/sorting.py` (crowding 流用)、 `breeding.py:211` (binary tournament 構造)、 `optimize.py:2125,2141` (主選抜 B-pooled 同等)
- Deb 2000 / Deb et al. 2002 学術引用済 (DOI 形式)

## レビュー観点 (Falsification-first、 詳細 Round 1)

1. **コード骨子の正当性 (synthesis § 7.1 / § 6.5 厳密準拠)**
   - 3 dataclass (ParetoAxis / IndividualEvaluation / GenerationSelectionResult) の field 構成
   - INVARIANT_VIOLATION_PENALTY=1.0e6 の数値妥当性 (smoke 後再校正候補)
   - extract_pareto_axis の f1/f2/f3 source 確定 (b_pooled_cf.net_pnl_after_cost / pooled_dd_per_fold_max / mission_inf_gap)
   - constrained_dominates の 3 段 (feasibility-first / infeasible-violation / feasible-Pareto)

2. **finite domain 防御 (Round 1 [C2] / [C3] 反映)**
   - extract_pareto_axis 内 `math.isfinite` 3 軸 assert
   - compute_effective_constraint_violation の constraint_violation +inf で ValueError raise
   - これらが defense-in-depth で T061/T062/T064 contract と整合しているか

3. **non_dominated_sort のアルゴリズム正当性 (Deb et al. 2002 fast non-dominated sort)**
   - O(M*N^2)、 dominated_set / domination_count の更新順序
   - constrained_dominates を両方向で呼ぶことで dominate 関係の双方向性を扱う

4. **crowding_distance の規約**
   - front_size <= 2 全員 +inf (Round 1 [W5])
   - 軸範囲 0 寄与 0 (NaN 防御)
   - 端 +inf 規約 (zenigame sorting.py:99-104 同等)
   - 3 軸正規化の累積加算

5. **select_survivors の rng 不要性と入口契約**
   - rng なし、 sort key (front_no, -crowding, genome_hash) の lex で deterministic
   - T064 contract 違反 2 ケースで ValueError raise
   - eligible が空 → 空 tuple 返し (warning なし、 caller 責務)

6. **binary_tournament の crowded-comparison only (Round 2 [Critical] 反映)**
   - sort_keys だけで比較、 evaluations 引数なし
   - constrained-domination の再評価しない
   - sort_keys は run_generation_selection で 1 回事前計算

7. **select_parent_pair の self-mating fallback**
   - max_retry=3 (synthesis § 7.5)、 超過で `(p1, p1)` 返し
   - caller (Phase 2 breeding) で `pair[0] == pair[1]` を mutation only シグナルとして消費する合意

8. **run_generation_selection の入口契約 fail-fast 4 ケース**
   - pop_size < 1 / population 空 / a_pass=True かつ bc_result is None / a_fail かつ bc_result is not None
   - eligible=0 で空結果返し (warning + 部分結果)、 ValueError しない

9. **GenerationSelectionResult の immutability**
   - frozen dataclass + MappingProxyType + frozenset + tuple
   - 外部からの mutation 経路がないか

10. **make_selection_seed の deterministic 性**
    - blake2b 8-byte digest → int.from_bytes (big endian, unsigned)
    - payload は json.dumps([run_id, gen_no, "selection"], separators=(",", ":")) で構造化
    - delimiter 衝突 (run_id 内 "|" 等) 回避
    - process salt なし (PYTHONHASHSEED 依存なし)

11. **テスト計画 13 sub-suite × 約 60 件**
    - PR DoD 必須 2 件 (a_fail / b_invariant_fail 除外)
    - finite check ValueError test (3 軸 + constraint_violation +inf + invariant_count negative)
    - delimiter 衝突 test (`run_id_with_pipe_does_not_collide`)
    - crowded-comparison only test (binary_tournament が constrained-domination 再評価しない)
    - sample_size_warnings 全コード列挙
    - immutability test (MappingProxyType / frozenset / tuple)
    - Determinism (same_seed_same_result_byte_for_byte)
    - test 漏れがあるか (例: non_dominated_sort の degenerate case、 select_parent_pair の sort_keys 受け渡し)

12. **C2 parallel-path 5 段階 grep DoD**
    - 段階 1 直 import / 段階 2 alias / 段階 3 relative / 段階 4 再エクスポート / 段階 5 runtime 配線
    - Phase 1 期待 0 件 (テスト以外) で確実か

13. **Phase 2 申し送り 8 箇所**
    - ファイル名 / 担当 TODO 名 / 内容 が具体的か
    - 漏れの可能性 (例: existing run_ga.py の旧 selection 経路全廃で削除すべき関数)

14. **Python 3.13 / ruff / mypy 対応**
    - frozen dataclass + types.MappingProxyType 型アノテーション
    - `__future__ annotations` (forward ref)
    - ジェネリクス型 (Mapping[int, ParetoAxis] 等) の整合性
    - `random.Random` 型ヒント (PEP 484 準拠)

15. **コーディングルール準拠**
    - 全施策テスト必須 (テスト計画で網羅済か)
    - バグ修正テストファースト (新規実装のため非該当)
    - uv 必須 (Phase 2 で `uv run pytest tests/alpha_factory/ga/test_nsga2_selection.py` 確認)

16. **Decision Pending 6 件の妥当性**
    - R1 INVARIANT_VIOLATION_PENALTY smoke 観測 DoD 完全か
    - R2 non_dominated_sort パフォーマンス (純 Python で smoke 完走可能か、 Numba JIT 検討タイミング)
    - R3 ε 厚 tie-break / R4 k=2 vs k=3 / R5 parent_pairs 長さ / R6 zenigame `optimize.py:2125,2141` 整合

17. その他、 詳細設計の弱点・盲点 (実装時の誤読リスク、 数値計算の数値安定性、 race condition、 名前衝突等)
...
| # | 施策名 | 変更ファイル | 優先度 |
|---|--------|------------|--------|
| 1 | `nsga2_selection.py` 新規 (3 dataclass + 1 const + 7 helper + 1 top-level entry + 1 seed helper) | `src/alpha_factory/ga/nsga2_selection.py` (新規) | Critical |
| 2 | `tests/alpha_factory/ga/test_nsga2_selection.py` 新規 (constrained-domination / Pareto axis / non-dominated sort / crowding / survivor / tournament / determinism / edge cases / contract violation / 全 6 sub-suite) | (新規) | Critical |
...
def non_dominated_sort(
    ...
    O(M*N^2) where M = number of objectives (3) and N = |eligible|.
...
def select_survivors(
    ...
    *,
    ...
    """rank → -crowding → genome_hash の lex 昇順で deterministic survivor 抽出.

    rng 不要 (Round 1 [W3])、 完全 deterministic.
    ...
    入口契約: T064 contract 違反 2 ケースで ValueError raise (Round 1 [W1] / Round 2 [W2]).
    """
    if target_size < 1:
...
def binary_tournament(
    ...
    *,
    ...
    {idx: (rank, -crowding, genome_hash)} を渡す。 tournament 内では tuple lex 比較のみ
    (constrained-domination の再評価はしない、 Round 2 [Critical]).
...
def select_parent_pair(
    ...
    *,
    ...
    p1 を 1 回抽出後、 p2 を最大 max_retry+1 回抽出して genome_hash 異なる相手を探す。
...
def run_generation_selection(
    ...
    *,
    ...
    sort_keys: dict[int, tuple[int, float, str]] = {
        idx: (front_no[idx], -crowding[idx], population[idx].genome_hash)
        for idx in eligible
    }
...
- Deb et al. 2002 fast non-dominated sort: O(M*N^2)
- pop=192-256、 M=3 → 約 200K 比較 / 世代 → 64 世代 / Run × 6 Run / epoch ≈ 80M 比較 / epoch
- 純 Python 実装で十分 (1 epoch あたり数秒以内、 smoke で要 profile)
- Phase 2 で profiling → 必要なら Numba JIT 検討 (Decision Pending、 § 残論点)
...
- `test_make_selection_seed_run_id_with_pipe_does_not_collide` (Round 2 [Suggestion 1] delimiter 衝突)
...
```
期待: 0 件 (テスト以外)。 テスト (`tests/alpha_factory/ga/test_nsga2_selection.py`) のみ hit。
...
## Phase 2 (T066 / T067 / T070 / T071 と同時、 別 PR) DoD 申し送り (8 箇所)
...
- [ ] § Phase 2 申し送り 8 箇所が具体ファイル名 + 担当 TODO 名で列挙
