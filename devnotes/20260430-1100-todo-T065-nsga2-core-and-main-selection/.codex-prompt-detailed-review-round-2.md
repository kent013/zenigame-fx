# T065 詳細設計 — Round 2 (詳細 Round 1 修正反映、 再レビュー依頼)

詳細 Round 1 で `CHANGES_REQUESTED`、 [Critical] 2 + [Warning] 5 + [Suggestion] 3 を全て反映済。

## 詳細 Round 1 → Round 2 対応マトリクス

| 詳細 Round 1 指摘 | Round 2 修正対応 | 該当箇所 |
|---|---|---|
| [C1] eligible==1 で binary_tournament が rng.sample(k=2) で ValueError、 self-mating fallback と矛盾 | `select_parent_pair` 冒頭で `len(survivors)==1` を早期 return `(survivors[0], survivors[0])`、 `len(survivors)==0` で ValueError raise (call site ガード)。 binary_tournament も `len(survivors) < 2` で ValueError raise (defense-in-depth) | select_parent_pair / binary_tournament |
| [C2] sort_keys 4-tuple 不足: 同一 genome_hash で Mapping 反復順依存、 byte-for-byte 再現性破綻 | sort_keys を `(front_no, -crowding, genome_hash, index)` 4-tuple に拡張、 IndividualEvaluation.index を最終 tie-break。 `eligible` 構築は `sorted(population.keys())` で安定順固定。 `_build_sort_keys` helper で型 `dict[int, tuple[int, float, str, int]]` 明記 | sort_keys (binary_tournament/select_parent_pair の引数型)、 _build_sort_keys、 _compute_eligible_axes_violations_feasibility |
| [W1] select_parent_pair docstring「max_retry+1 回抽出」 と擬似コード `range(max_retry)` の不一致 | docstring を「最大 max_retry 回追加抽出 (= 合計最大 max_retry+1 回 binary_tournament 呼出)」 と表記統一 | select_parent_pair docstring |
| [W2] non_dominated_sort i/j 全探索で比較 2 倍 | `i < j` のみ比較、 dominate 関係に応じて両側更新 (O(M*N(N-1)/2)) | non_dominated_sort |
| [W3] select_survivors と run_generation_selection の eligible/front/crowding 計算重複 | private helper `_validate_t064_contract` / `_compute_eligible_axes_violations_feasibility` / `_compute_front_assignments_and_crowding` / `_build_sort_keys` に抽出、 両 entry から DRY | private helpers section |
| [W4] C2 grep DoD 段階 1 で tests 含む点が曖昧 | 段階 1 コマンドで `--exclude-dir=tests` 明示、 別途 tests 配下の hit 確認 grep を分離 | C2 grep DoD 段階 1 |
| [W5] 「全 6 sub-suite」 と「13 sub-suite」 の記述不整合 | 全箇所「**13 sub-suite**」 で統一 | 施策 1 表 |
| [S1] INVARIANT_VIOLATION_PENALTY 事前上界根拠 | R1 残論点に「constraint_violation 上界目安 1.0e2、 1.0e6 ≫ 1.0e2 で支配性確保」 + 「invariant 万件は upstream pipeline 異常」 を 1 行 | R1 残論点 |
| [S2] テスト追加: survivors==1 / 同一 genome_hash 複数体 / non_dominated_sort 空入力 | テスト計画に 3 件追加 (`test_select_parent_pair_single_survivor_returns_self_pair` / `test_select_parent_pair_duplicate_genome_hash_falls_back_to_self_pair` / `test_non_dominated_sort_empty_input_returns_empty_dict` 等) + sort_keys 4-tuple test 3 件追加 | § 2.5 / § 2.9 / § 2.11 |
| [S3] Phase 2 申し送りに「旧 selection 経路の削除対象シンボル一覧」 明示 | 申し送り表に項目 #9 追加: stage_gate.py / archive.py / cross_pair.py / swim_lane.py / run_ga.py の selection 関連削除対象を grep で確定する DoD | Phase 2 申し送り #9 |

## Round 2 着目点 (確認のみ)

- `len(survivors)==1` で `(survivors[0], survivors[0])` 早期 return は `D8 self-mating fallback` 仕様と一致
- sort_keys 4-tuple `(front_no, -crowding, genome_hash, index)` で deterministic 完全保証
- `_compute_eligible_axes_violations_feasibility` 内 `sorted(population.keys())` で eligible が Mapping 反復順から独立
- private helpers の DRY 化が select_survivors と run_generation_selection で正しく共通化されているか
- non_dominated_sort 比較数が `N*(N-1)/2` に半減した実装が dominate 関係の対称性を破っていないか (i<j で 1 回比較、 dominate なら片側更新、 dominate されるなら反対側更新)
- C2 grep DoD 段階 1 が機械判定可能か (tests 除外明示)
- Phase 2 申し送り #9 で旧 selection 経路全廃の DoD が具体ファイル名 + grep コマンドで明示

## Round 2 verdict 期待

- APPROVED (詳細 Round 1 [Critical/Warning/Suggestion] 全反映、 数値安定性・determinism・Phase 2 整合の各課題解消)
...
|---|---|
| [C1] eligible==1 で `binary_tournament` の `rng.sample(..., k=2)` が ValueError、 D8 self-mating fallback と矛盾 | `select_parent_pair` 冒頭で `len(survivors) <= 1` を早期 return `(survivors[0], survivors[0])` で fallback、 binary_tournament 呼出ガード |
