...
前回 (round 1) の REVISE 指摘 5 点に対する対応を確認し、再判定 (APPROVED / REVISE) を出してください。

前回の必須修正点:
1. generation_weighted_mean の pass_rate 集計範囲と fitness_pen_pool 範囲を一致させる
2. stage_a_pass / generation / fitness_pen の null/NaN 方針を明文化、違反時 exit 8 統一
3. atomic write を一意 tmp 名 + 同時実行排他方針 (lock or single-writer) 明記
4. calibrate.* の転記経路を表で明示 (config → loader → consumer。genome.meta は不要なら理由)
5. smoke を 2 段に分離 (skip 経路 + 成功経路)

推奨改善点 (任意):
- Decision に effective_sample_size 追加
- monitoring 指標は因果解釈しない旨をログに明記
