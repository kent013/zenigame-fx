# Conceptual Review Round 3: stats-completeness-gate-stage-b

あなたは zenigame-fx Alpha Factory の改善に関する概念設計レビュアーです。Round 2 で 3 Critical 指摘を受けて改訂した Round 3 です。

【Round 2 で残った Critical】
1. [Critical] `run-report` の世代別 reason 集計に必要なデータ経路が設計にない → archive に `stage_b_reason_codes` 永続化
2. [Critical] underfilled 判定式が現行 WF 契約 (`make_wf_folds` の observed-day) と一致していない → `walk_forward.py` の SSOT helper を共有
3. [Critical] underfilled の根拠前提 P13 が未検証 → `make_wf_folds` 読解で Verified 化

【Round 3 改訂内容】
- P13 を Verified 化 (`fold_len = train_days + embargo_days + test_days` 観測日 SSOT 確認)
- P14 を新規追加 (Run 7-9 = 15 観測日 << 141 観測日要件、Verified)
- `wf_min_unique_dates()` helper を `walk_forward.py` に切り出し SSOT 共有
- LaneManager で observed-day 契約 (n_unique_dates) で判定
- archive に `stage_b_reason_codes` / `n_fold_effective` / `positive_fold_ratio_effective` を追加
- `stage_b_window_underfilled` を独立 canonical code として明示
- 実装順序を `契約判定 → 永続化 → 可視化` の 3 段に整理
...
- Round 2 Critical が解消されたかを各観点で必ず明示
