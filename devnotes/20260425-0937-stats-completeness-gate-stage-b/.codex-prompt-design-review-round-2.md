# Detailed Design Review Round 2: stats-completeness-gate-stage-b

Round 1 で **CHANGES_REQUESTED** (1 Critical, 7 Warning, 3 INCONCLUSIVE/REQUEST_CHANGES)。
本ラウンドで以下を反映済:

【Round 1 主要指摘の解消】
1. [Critical] `walk_forward.py` で `StageGateConfig` 循環 import → **解消**: helper 引数を `int` 3 つに変更、`StageGateConfig` 依存削除、`make_wf_folds` 内部 `fold_len` も同 helper を使う SSOT 統合
2. [Warning] `n_unique_dates(lane.bars_18m)` 個体ごと再計算無駄 → **解消**: lane ループ外で 1 回計算
3. [Warning] underfilled payload `is_full_*=0.0/0` が archive 上書きされる → **解消**: `None` に変更
4. [Warning] 既存 `test_swim_lane.py:511` fixture 期待が崩れる → **解消**: 既存テスト更新項目を施策 6 集約表に明記
5. [Warning] archive 28 列固定検査未更新 → **解消**: 施策 6 集約表に 31 列更新を明記
6. [INCONCLUSIVE] 旧 parquet 後方互換テスト未明示 → **解消**: `test_archive_load_old_schema_parquet_returns_none_for_new_columns` 追加
7. [Warning] run-report multi-label「先頭 reason のみ集計」過少計上 → **解消**: `primary_reason` + `any_reason incidence` の 2 系列出力
8. [Warning] 旧 archive (`stage_b_reason_codes` 無し) 扱い不明瞭 → **解消**: `unknown_reason` カテゴリ追加
9. [Warning] `tests/scripts/` 受け入れ条件未含 → **解消**: 受け入れ条件に追加
10. [Suggestion] `terminology.md` 任意 → 必須に格上げ
11. [Suggestion] skill SKILL.md 整合 → 波及変更に追加
