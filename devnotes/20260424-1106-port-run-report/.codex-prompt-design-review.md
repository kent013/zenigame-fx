# Detailed Design Review: T021 zenigame-fx-run-report skill port

あなたは zenigame-fx Alpha Factory の詳細設計レビュアーです。
gpt-5.3-codex high reasoning として、実装直前の technical design review を行ってください。

## レビュー観点

1. **CLI 仕様**: action="append" のデフォルト None → [] 正規化が正しいか
2. **mypy 通過性**: 各 builder の型が pyarrow / dict / Mapping で破綻しないか
3. **pyarrow API**: group_by + aggregate の戻り値型、sort_indices の使い方が正しいか（pyarrow 14+/15+ 想定）
4. **Best 補足 lookup**: name + generation で archive 全体検索する際、複数 lane の同名個体が引かれた場合の表示順序
5. **defensive 動作**: archive 読取失敗時の例外捕捉が `Exception` で良いか、もう少し絞るべきか
6. **テスト網羅**: 17 テストで漏れている観点（特に異常系）はないか
7. **既存 test_alpha_factory_run_ga.py への影響**: 本当に無いか
8. **markdown 出力の冪等性**: テーブル整形・改行で diff が安定するか
9. **SKILL.md の grep 検証**: 必須見出し 12 個のうち archive 不在時に欠けるものをどう許容するか

## 出力

Round 判定: APPROVED / NEEDS_REVISION
NEEDS_REVISION の場合は具体的な指摘（最大 5 個、優先度付き）。
具体的なコード修正案も併記してください。
