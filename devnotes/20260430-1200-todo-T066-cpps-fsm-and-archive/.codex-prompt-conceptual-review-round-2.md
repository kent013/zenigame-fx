# T066 概念設計 — Round 2 (Round 1 修正反映、 再レビュー依頼)

Round 1 で `CHANGES_REQUESTED`、 [Critical] 5 + [Warning] 6 + [Suggestion] 5 を全て反映済。

## Round 1 → Round 2 対応マトリクス

| Round 1 指摘 | Round 2 修正対応 | 該当 § |
|---|---|---|
| [C1] archive_admit SSOT 崩壊 | dataclass 2 層分離 (Suggestion 1 採用): ArchiveCandidate (admission 前、 全 source field) + ArchiveMember (admission 後、 archive_role 3 種類のみ + archive_target 確定)。 § 3.1 / § 8 / § 11.2 で `Mapping[str, ArchiveCandidate]` に統一 | § 1 / § 8.1.1-8.1.2 / § 8.2 / § 11.2 |
| [C2] dataclass field 欠落 (gate_worst_gap, run_id_index, new_dataset_epoch_id) | ArchiveCandidate に gate_worst_gap / dataset_epoch_id / genome_id / pattern_id / family_id 等を必須化。 update_archive_per_run に new_dataset_epoch_id 引数追加。 run_id_index は ArchiveState.run_history から pure 計算化 | § 8.1.1 / § 8.6 / § 11.2 / § 9.1-9.2 |
| [C3] CA/DA 流入責務未定義 | **設計判断**: T066 Phase 1 archive_admit は **CA admission のみ**。 DA admission は T067 warmstart 経路で別途。 archive_target は T066 が「mission/progress/score_bypass は全て CA」 と決定。 synthesis Round 22 改訂候補として § 13 R8 に追加 | § 1 / § 8.5 / § 13 R8 |
| [C4] 強制遷移値不整合 (§1.1/§5.2: 44 / §4.1: pop_gen=64 else 30) | force_g は caller 引数で渡す方針に統一 (config 由来)、 「gen=64 baseline → 44、 gen=48 fallback → 再校正」 を default として明示。 概念設計内で hardcode しない | § 4.1 / § 5.2 / § 11.2 |
| [C5] deterministic 不足 | CA / DA eviction lex 末尾に genome_id 追加 (lex 9 段 / 8 段に拡張)。 archive_admit は genome_id 昇順で sorted candidates、 同一スコア tie は genome_id 昇順で決定論化 | § 8.2 / § 9.1-9.2 |
| [W1] capacity API pop>=2 vs 192/256 限定 | 全 capacity 系 API を 192/256 限定に統一、 他 pop_size は ValueError raise | § 11.2 |
| [W2] target_inflow 丸め規約 | 192/256 固定値 (target=8/10、 per_run_max=12/16) を SSOT、 0.04*pop / 0.06*pop は導出規約として注記 | § 1.1 |
| [W3] partition_survivors_to_ca_da が sort_keys 未使用 | sort_keys 引数削除、 「T065 sort 順前提」 を docstring 明示 | § 4.1 / § 11.2 |
| [W4] recency_floor=12 「保証」 vs 12 未満時不可 | best-effort 化、 AdmissionReport.recency_floor_unmet=True で warning 通知、 強制保持なし | § 9.3 |
| [W5] per_run_max 超過時の trim 未定義 | apply_hard_constraints で階層優先 + admission key 順 + genome_id 末尾の trim 規則明示 | § 8.3 |
| [W6] quality_floor_margin と margin_inf p70 threshold 混線 | 名前分離: 品質床は `margin_inf_passes_p70: bool` (caller 計算済 bool)、 DA eviction #5 は `quality_floor_margin: float` (caller 別計算)。 両者独立 | § 8.4 |
| [S1] 入力 2 層分離 | 採用 (Critical 1 と統合) | — |
| [S2] update_archive_per_run に new_dataset_epoch_id | 採用 (Critical 2 と統合) | — |
| [S3] eviction lex 末尾 genome_id、 run_id_index pure 計算 | 採用 (Critical 5 と統合) | — |
| [S4] not_score_bypass 導出値 | ArchiveMember.not_score_bypass を property (`archive_role != "score_bypass"`) で導出、 保存値削除 | § 8.1.2 |
| [S5] c_pass_depth INCONCLUSIVE、 T064 SSOT 改訂を先 | **Phase 0 確定**: T064 follow-up PR (BCEvaluationResult.c_pass_depth 追加) を T066 詳細設計より先に着地必須。 § 13 R9 に詳細 | § 13 R9 / § 14 #7 |

## Round 2 着目点 (確認のみ)

- ArchiveCandidate / ArchiveMember 2 層モデルが § 8.1.1-8.1.2 + § 11.2 で SSOT 統一
- CA admission only 設計判断 (§ 8.5) と synthesis § 8.2 解釈の関係 (synthesis Round 22 改訂候補として § 13 R8)
- update_archive_per_run の epoch reset 検出 (§ 8.6) で T058 dataset_epoch_id 契約準拠
- CA / DA eviction lex 末尾 genome_id 追加で deterministic 完全保証 (run_history 経由 pure 計算)
- recency_floor best-effort 化 (§ 9.3) と AdmissionReport.recency_floor_unmet 通知
- per_run_max 階層別 trim 規則 (§ 8.3 ARCHIVE_ROLE_PRIORITY)
- T064 follow-up Phase 0 (c_pass_depth field 追加) が T066 詳細設計の前提

## Round 2 verdict 期待

- APPROVED (Round 1 全反映、 SSOT 統一 + CA only 設計判断 + Phase 0 依存明示)
...
### 4.2 per-Run (archive 更新、 Round 1 [C1] / [C2] / [C3] 反映)
...
- new_dataset_epoch_id: str                    # epoch 切替時に変更、 prev と不一致なら archive reset (Round 1 [C2])
- new_run_id: str                              # 新 Run の ID、 archive_state.run_history に追加

処理:
1. epoch 切替検出: prev_archive.dataset_epoch_id != new_dataset_epoch_id
   → archive 全リセット (members=()、 run_history=())
2. 候補集合は既に ArchiveCandidate (admission 前) として全 source field 確定済 (caller 責務)
3. 各候補の archive_role 確認 (= determine_archive_role(bc_result))
4. inflow_targets = compute_inflow_targets(pop_size, n_mission, n_progress, mode=mode)
5. CA admission のみ実施 (Round 1 [C3] 解消、 § 8.5 設計判断):
   - mission_pass: 全件 (per_run_max まで)
   - progress_pass: gate_worst_gap 昇順で remaining target まで
   - score_bypass: bypass_k 体を gate_worst_gap 昇順 (品質床: invariant_feasible AND margin_inf_passes_p70)
   各 admission で ArchiveCandidate → ArchiveMember (archive_target="CA") に変換
6. ハード制約適用: per_run_max / pattern_max_share=0.25 / 階層内残留 trim
7. CA evict: capacity 超過分を CA lex 末尾から削除 (recency_floor=12 best-effort)
8. DA evict: capacity 超過分を DA lex 末尾から削除 (DA admission は本 entry では行わない、 T067 別経路)

出力:
- new_archive, AdmissionReport
```

`AdmissionReport` の field:
- `admitted_genome_ids: tuple[str, ...]`
- `evicted_genome_ids: tuple[str, ...]`
- `n_mission / n_progress / n_bypass: int`
- `mode: Literal["normal", "emergency"]`
- `hard_constraint_drops: int`
- `recency_floor_unmet: bool`     (Round 1 [W4] best-effort)
- `dataset_epoch_reset: bool`     (epoch 切替時 True)
...
def archive_admit(
    ...
    *,
    ...
    # 7. archive merge (新 members 追加 + run_history 更新)
    new_members = archive_state.members + tuple(admitted_filtered)
    ...
    # 9. AdmissionReport 構築
    admitted_ids = tuple(m.genome_id for m in admitted_filtered)
    evicted_ids = tuple(
        m.genome_id for m in archive_state.members
        if m.genome_id not in {nm.genome_id for nm in new_state.members}
    )
    ...
    return new_state, AdmissionReport(
        ...
        n_mission=len(mission_admits),
        n_progress=len(progress_admits),
        n_bypass=len(bypass_pool_sorted),
        mode=mode,
        hard_constraint_drops=hard_drops,
        recency_floor_unmet=recency_unmet,
        dataset_epoch_reset=False,                           # update_archive_per_run で reset 検出時に True
    )
...
### 8.6 epoch 切替時のリセット (Round 1 [C2] 反映)
...
def update_archive_per_run(
    ...
    *,
    ...
    """top-level entry: epoch reset 検出 + admission + eviction を 1 関数で.
    ...
    - 同一 epoch なら通常 admission/eviction
    """
    if prev_archive.dataset_epoch_id != new_dataset_epoch_id:
        empty_archive = ArchiveState(
            members=(), dataset_epoch_id=new_dataset_epoch_id, run_history=(),
        )
        new_state, report = archive_admit(empty_archive, candidates, pop_size=pop_size, mode=mode)
        report = replace(report, dataset_epoch_reset=True)
        return _append_run_history(new_state, new_run_id), report

    new_state, report = archive_admit(prev_archive, candidates, pop_size=pop_size, mode=mode)
    return _append_run_history(new_state, new_run_id), report
```
...
### 9.1 CA eviction (lex 9 段、 synthesis Round 21 改訂後 + Round 1 [C5] / [S3])
...
def _ca_eviction_sort_key(
    ...
    *,
    ...
    Round 21 改訂後 + Round 1 [C5] (lex 末尾 genome_id) + Round 1 [S3] (run_id_index pure 計算):

    1. mission_pass (True が上位)
    2. progress_pass (True が上位)
    3. not_score_bypass (= archive_role != "score_bypass"、 True が上位、 m.not_score_bypass property で導出)
    ...
    9. genome_id (昇順、 deterministic 完全保証 tie-break、 Round 1 [C5])
    """
...
def _da_eviction_sort_key(
    ...
    *,
    ...
    3. progress_pass (True が上位)
    4. not_score_bypass (True が上位、 property で導出)
    5. quality_floor_margin (大が上位)
    6. recency (新が上位)
    7. log_pf_clip (大が上位)
    8. genome_id (昇順、 deterministic tie-break、 Round 1 [C5])
    """
    run_id_index = run_history.index(m.run_id) if m.run_id in run_history else len(run_history)
    return (
        -m.novelty,
