# T066 概念設計 — Round 3 (Round 2 修正反映、 再レビュー依頼)

Round 2 で `CHANGES_REQUESTED`、 [Critical] 5 + [Warning] 2 + [Suggestion] 4 を全て反映済。

## Round 2 → Round 3 対応マトリクス

| Round 2 指摘 | Round 3 修正対応 | 該当 § |
|---|---|---|
| [C1] recency 実装順序破綻 (archive_admit 内 eviction で run_history 未登録、 当 Run 新規が「最古」 扱い) | update_archive_per_run で **run_history を archive_admit より先に更新**。 1) candidate epoch 検証 → 2) epoch reset 判定 → 3) run_history に new_run_id を先頭追加 (maxlen=10) → 4) archive_admit (eviction 内で更新済 run_history を参照) | § 8.6 |
| [C2] ArchiveMember に mission_pass / progress_pass field なしに eviction key で参照 | eviction key を archive_role から完全導出 (`is_mission = m.archive_role == "mission_pass"` 等)、 ArchiveMember の bool field 追加なし | § 9.1 / § 9.2 |
| [C3] § 3.1 / § 7 が旧 API のまま (sort_keys あり、 Mapping[int, IndividualEvaluation]) | § 3.1 を「§ 11.2 SSOT 参照」 の概略のみに変更、 § 7 partition_survivors_to_ca_da から sort_keys 引数削除、 「T065 sort 順前提」 を docstring 明示 | § 3.1 / § 7 |
| [C4] AdmissionReport に recency_floor_unmet / dataset_epoch_reset field なし | § 10 dataclass 定義に 2 field 追加 (bool)。 § 8.2 admission 内で recency_unmet 計算、 § 8.6 epoch reset で True 上書き | § 10 |
| [C5] compute_ca_da_capacities が `pop_size < 2` ガード残存、 192/256 限定と矛盾 | `SUPPORTED_POP_SIZES = (192, 256)` 定数化、 `pop_size not in SUPPORTED_POP_SIZES` で ValueError raise | § 5.3 |
| [W1] evicted_genome_ids が「旧 archive から消えたもののみ」 算出、 admit→即 evict 漏れ | 算出を「`旧 archive members + 当 Run admit` の union から `new_state.members` を引いた差分」 に変更、 admit→即 evict も計上 | § 8.2 step 9 |
| [W2] epoch reset 時に candidates の dataset_epoch_id 検証なし | update_archive_per_run 入口で `c.dataset_epoch_id == new_dataset_epoch_id` 全件検証、 不一致は ValueError raise | § 8.6 |
| [Suggestion 1] update_archive_per_run で先に run_history 反映 | 採用 (Critical 1 と統合) | — |
| [Suggestion 2] ArchiveMember に bool 保持 OR archive_role 導出に統一 | 後者採用、 archive_role から完全導出 | — |
| [Suggestion 3] § 3.1 / § 7 を SSOT 同期 | 採用 (Critical 3 と統合) | — |
| [Suggestion 4] compute_ca_da_capacities も 192/256 のみ ValueError | 採用 (Critical 5 と統合) | — |

## Round 3 着目点 (確認のみ)

- update_archive_per_run の処理順序: epoch 検証 → epoch reset → run_history 更新 → archive_admit
- ArchiveMember は mission_pass / progress_pass field なし、 eviction key 内 archive_role 導出 (m.archive_role == "mission_pass" 等)
- evicted_genome_ids が admit→即 evict も計上 (new_state_ids 集合からの差分)
- AdmissionReport に recency_floor_unmet / dataset_epoch_reset の 2 bool field 追加
- compute_ca_da_capacities が SUPPORTED_POP_SIZES 定数で 192/256 のみ正式
- candidates dataset_epoch_id 全件検証で epoch 汚染防止

## Round 3 verdict 期待

- APPROVED (Round 2 [Critical 5] / [Warning 2] / [Suggestion 4] 全反映)
- CHANGES_REQUESTED
- BLOCKED
...
class AdmissionReport:
    """archive_admit の結果サマリ (observability)、 詳細 Round 2 [C4] field 補完.
