# T066 概念設計 — Round 4 (Round 3 修正反映、 再レビュー依頼)

Round 3 で `CHANGES_REQUESTED`、 [Critical] 2 + [Warning] 2 + [Suggestion] 3 を全て反映済。

## Round 3 → Round 4 対応マトリクス

| Round 3 指摘 | Round 4 修正対応 | 該当 § |
|---|---|---|
| [C1] genome_id 重複時 capacity 破綻 (eviction の survivor_ids set で同 ID 全件残存) | `ArchiveState.members` の **genome_id 一意制約** を文書化、 archive_admit の merge を **upsert** (`existing_by_id[nm.genome_id] = nm` で既存置換) に変更。 § 10 ArchiveState docstring + § 8.2 step 7 で明記 | § 10 / § 8.2 |
| [C2] lex SSOT 不一致 (§9.1/9.2 で 9段/8段 だが §1.2/1.3 は 8段/7段、 §12.8 テスト名旧) | § 1.2 / § 1.3 を 9 段 / 8 段 (genome_id 末尾) に更新。 § 12.8 テストを `genome_id_is_final_tiebreak` 追加 + `log_pf_clip_is_position_8_not_final` でリネーム | § 1.2 / § 1.3 / § 12.8 |
| [W1] n_mission / n_progress / n_bypass が selected vs admitted のどちらか不明 | AdmissionReport を `n_selected_*` / `n_admitted_*` の 6 field に分離。 selected = hard constraint 前 (候補階層選抜時点)、 admitted = hard constraint 後 (実流入数) | § 10 / § 8.2 step 9 |
| [W2] § 4.2 と § 8.6 の「run_history 先反映」 修正同期不足 | § 4.2 を § 8.6 SSOT に同期 (1. epoch 検証 → 2. epoch reset → 3. run_history 更新 → 4. archive_admit) | § 4.2 |
| [Suggestion 1] genome_id 一意制約明文化 + upsert | 採用 (Critical 1 と統合) | — |
| [Suggestion 2] lex 最終段の文書同期 | 採用 (Critical 2 と統合) | — |
| [Suggestion 3] selected vs admitted 分離 | 採用 (Warning 1 と統合) | — |

## Round 4 着目点 (確認のみ)

- ArchiveState.members の genome_id 一意制約 invariant が docstring + 実装 (upsert) で明示
- § 1.2 (CA 9 段) / § 1.3 (DA 8 段) / § 9.1 / § 9.2 の lex 順序が完全一致、 末尾は genome_id
- AdmissionReport の selected_* / admitted_* 6 field 分離
- § 4.2 と § 8.6 の処理順序記述が同期 (run_history 先反映)
- § 12 テスト計画に新規 3 件追加 (upsert / admit→即 evict / selected vs admitted)

## Round 4 verdict 期待

- APPROVED (Round 3 [Critical] 2 / [Warning] 2 / [Suggestion] 3 全反映、 SSOT 統一 + genome_id 一意制約確立)
- CHANGES_REQUESTED
- BLOCKED
