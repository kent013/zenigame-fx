...
Round 2 の Critical / Warning に対応しました。

### Round 2 指摘 → 対応マトリクス

| Round 2 指摘 | 対応 |
|--------------|------|
| [Critical] Tier-A の定義矛盾（即時有効と謳いつつ script 未実装） | Tier 区分を撤廃。代わりに `port_status` (rename_done) と `executable` (executable / abort_only) の 2 軸に分離。本 TODO は port_status のみ対象。4 skill すべて executable=abort_only と明記し、これは設計意図と宣言 |
| [Warning] snapshot の未充足時挙動が閉じていない | 依存マトリクスを書き換え: 「skill としては snapshot_manager.py 不在ならアボート一択。artifact 不足時の config-only 動作判定は scripts/alpha_factory/snapshot_manager.py 側の責務（本 skill の範疇外）」と分離 |
| [Warning] post-run-review が batch-ga の依存マトリクスから抜けている | 「batch 中スキップ対象なので依存ではない、注意のみ」と明記 |
| [Suggestion] Tier 1 軸ではなく移植状態 / 実行可能性を分離 | 採用 |
| [Suggestion] clear-cache の固定文 | 「実 namespace は scripts/cache_manager.py status を source of truth とする」を固定文として skill に入れる旨を明記 |

## レビュー観点（Round 3）

Round 2 指摘が反映されたか確認してください。
