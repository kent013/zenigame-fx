T044 (Stage B Feasibility Contract Phase 1) 実装レビュー。

# 背景
cycle 3 (Run-13) で Stage B failure reason = insufficient_folds 100% 単峰判明。WF 必要観測日数 > Stage B window で構造的 fold 不成立。cycle 3 で WF パラメータ短縮 (60/10) 対症療法、cycle 5 で T035 known_codes 漏れ修正で真因 (median_oos_sharpe<min) 露出、cycle 7 で T031 entry_count_min 1→50 で trade_count 大幅改善。本 T044 で構造的 pre-flight feasibility contract を導入。

# 7 施策
1. walk_forward.compute_max_folds(n, train, embargo, test, step) helper
2. StageGateConfig.wf_min_folds_required: int = 2
3. config.py loader 反映 + default.yaml 追記
4. LaneManager で max_folds<min なら全 lane skip with reason_codes=("stage_b_pre_flight_underfilled",) — T035 wf_min_unique_dates skip-path より優先
5. generate_run_report known_codes に追加
6. テスト 9 件 (walk_forward 4 + config 2 + swim_lane 2 + report 1)
7. docs (Phase 2 で更新予定、本 cycle は skip)

# 検証
- pytest: 1070 passed, 1 skipped (T044 +9 件)
- ruff: T044 関連ファイル全 clean
- mypy: 90 files clean
...
@@ -494,8 +505,39 @@ class LaneManager:
             if not a_result.passed:
                 continue
             stage_a_pass += 1
-            # Stage B (T035: 観測日数充足検査 → underfilled なら skip-path)
-            if lane_n_unique_dates < wf_min_dates:
+            # Stage B (T035: 観測日数充足検査 / T044: max_folds 検査 → underfilled なら skip-path)
+            if preflight_underfilled:
+                # T044: max_folds < min_folds_required → fail-fast
+                # T035 の wf_min_unique_dates 経路より優先 (より informative)
...
index f75a546..afbc8a6 100644
--- a/tests/alpha_factory/test_swim_lane.py
+++ b/tests/alpha_factory/test_swim_lane.py
@@ -959,3 +959,53 @@ def test_stage_b_evaluated_when_unique_dates_meet_minimum(
     assert counts["a"] == 2
     # Stage B は通常通り評価される
     assert counts["b"] == 2
+
...
# レビュー観点
1. 設計との一致 (devnotes/20260426-0957-stage-b-feasibility-contract/detailed-design.md, ID は T044 にリネーム)
2. T035 wf_min_unique_dates skip-path との排他順 (T044 を先に判定)
3. 4 段伝搬 (config.py loader → StageGateConfig → LaneManager → archive payload)
4. テスト網羅性
5. 禁止事項違反なし
