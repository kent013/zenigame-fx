【思考原則】仮説立て / データに真摯 / 先人の知恵 / 機能の名前 / 値より構造 / 因果ループ切断禁止。
...
あなたは zenigame-fx Alpha Factory の skill 実装レビュアーです。

【本件】
T022「skill port: zenigame-fx-plan-and-design」の実装レビュー。
Markdown ファイル 1 個（`.claude/skills/zenigame-fx-plan-and-design/SKILL.md`）の新設 + 波及変更（`.claude/skills/zenigame-fx-codex-review/SKILL.md` の session_label 命名規則に `todo-selection` 追加）のみ。

【検証済】
- grep -cE "docs/alpha-factory|src/trading|/zenigame-codex|~/.local/bin/codex-vscode|/zenigame-alpha-design|/zenigame-todo-add|/zenigame-analyze-run|/zenigame-post-run-review" .claude/skills/zenigame-fx-plan-and-design/SKILL.md → 0
- SKILL.md 752 行
- 詳細設計は Codex design-review Round 2 で APPROVED 済（`devnotes/20260424-1138-port-plan-and-design/design-review-round-2.md`）
- 詳細設計の Round 1 Critical（obsolete コマンド呼び出し契約）は修正済

【レビュー観点（impl-review、md only）】
1. SKILL.md の YAML frontmatter（name / description / argument-hint）が valid か
2. 詳細設計（detailed-design.md）と SKILL.md 本文に齟齬がないか
3. 既存 zenigame-fx-* skill との参照整合性（呼び出し contract）
4. zenigame の参照（/zenigame-codex 等）が混入していないか（grep 済）
5. session_label `todo-selection` の波及変更が codex-review 側に確実に追加されたか
6. 他に implement 段階で見落としがあるか
...
## 実装ファイル: .claude/skills/zenigame-fx-plan-and-design/SKILL.md
...
## 波及変更: .claude/skills/zenigame-fx-codex-review/SKILL.md (session_label セクション)
