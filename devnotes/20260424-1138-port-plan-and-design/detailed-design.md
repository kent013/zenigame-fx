# 詳細設計: skill port — zenigame-fx-plan-and-design

## 使命・制約（絶対遵守）

### zenigame-fx Alpha Factory 使命

`config/alpha_factory/default.yaml` の `live_criteria` を全て満たす FX イントラデイ戦略ゲノム個体を 1 つ見つけ出す。Cross-pair (ii-lite) も同時通過。
...
### コーディングルール（本件は md only）

- **本件は SKILL.md 1 ファイル新設のみ**。Python コード変更なし
- ruff / mypy / pytest 影響範囲: なし（md ファイル）
- 受け入れ検証: §「検証」の grep + セクション存在確認

## 概念設計リファレンス

`devnotes/20260424-1138-port-plan-and-design/conceptual-design.md`

## 施策一覧

| # | 施策名 | 変更ファイル | 優先度 |
|---|--------|------------|--------|
| D1 | `.claude/skills/zenigame-fx-plan-and-design/SKILL.md` 新規作成 | 新規 | High |

---

## D1: zenigame-fx-plan-and-design SKILL.md 新規作成

### 変更箇所

- ファイル: `.claude/skills/zenigame-fx-plan-and-design/SKILL.md`（新規）

### 波及変更

- `AGENTS.md`: 既存 zenigame-fx-* skill 一覧に「zenigame-fx-plan-and-design」行を追加（必要なら。skill descriptor は `.claude/skills/` から自動 discoverable のため必須ではない）
- `.claude/skills/zenigame-fx-codex-review/SKILL.md`: §session_label 命名規則に **`todo-selection`** を追記（移植元 zenigame では暗黙だったが、本 skill が新規 label として使うため明示）
- `.claude/skills/zenigame-fx-improve-cycle/SKILL.md`: 変更なし（呼び出し経路接続は別 follow-up TODO）
- `.claude/skills/zenigame-fx-autopilot/SKILL.md`: 変更なし（同上）
- `config/alpha_factory/default.yaml`: なし
- `docs/alpha_factory/*.md`: なし
- `config/alpha_factory/focus-theme.json`: 新規作成しない（不在時 fallback で動作）

### 現行コード

該当ファイル `.claude/skills/zenigame-fx-plan-and-design/SKILL.md` は **未存在**。

### 変更後コード

新規 SKILL.md 全文を以下に示す。実装時はこの内容をそのまま `.claude/skills/zenigame-fx-plan-and-design/SKILL.md` として書き出す。
...
`scripts/alpha_factory/todo_manager.py` で Open テーブルを取得:
...
# list は Open / Conditional 両方を返す。Conditional セクションを抽出
```
...
**standalone サイクルの場合**: `analysis-merged.md` 作成後、状態ファイルの `deferred_analyses` リストにこのパスを追記。
...
|------|------------------|----------|--------------|
| `zenigame-fx-post-run-review` | A-1 滞留判定の「post-run-review に委ねる」分岐、Phase D 後段の自動起動 | **no-op、成果物要求なし** | A-1 で `Read` 可能になり次第、滞留 TODO の再評価を委譲 |
...
- [x] バグ修正ではない（新規 skill 追加）
- [x] 既存テストへの影響なし（md only）
- [ ] 新規テスト: なし（skill ファイルは md only。動作テストは将来的に improve-cycle / autopilot 統合時に E2E で実施）
- [x] **検証コマンド**:
  ```bash
  ...
  期待値: **0**
- [x] **必須セクション存在確認**:
  - 引数表 / 入力 / 出力一覧 / 責務境界 / 呼び出し契約 / 使命継承宣言 / 状態ファイル更新責務 / 合議ループ収束ルール / focus-theme fallback / Phase A / Phase B / Phase C / エラーハンドリング / 未接続 hook / 使用例

### リスク

| リスク | 重要度 | 対策 |
|-------|------|------|
| 合議長文化により mission 逸脱（文書整合性最適化） | 中 | §合議ループ収束ルール（強制終結 + ラウンド数短縮 10→5） |
| 継承先 `zenigame-fx-codex-review` の節名変更で挙動暗黙変化 | 中 | §使命継承宣言で必須節名固定、波及変更チェック対象明示 |
| `focus-theme.json` 不在で一般論に流れる | 低 | §focus-theme fallback で `general` 固定、選定基準 0 をスキップ |
| `skip_todos` 判定 `design stale` 大量発生で TODO 枯渇 | 低 | post-run-review 整備後にリビルド（本 skill 範囲外、コメント化） |
| `zenigame-fx-improve-cycle` / `zenigame-fx-autopilot` が本 skill を呼ばない（接続未整備） | 中 | 呼び出し契約で「現状 manual / 将来 improve-cycle」明記、follow-up TODO 化 |
| `todo_manager.py list` が Open / Conditional 両方を返す形式（旧 zenigame は別 subcommand） | 低 | A-1 で「list を呼んで Conditional セクションを抽出」と明記 |
| Conditional 自動昇格 subcommand `promote-conditional` が `todo_manager.py` に未提供 | 低 | A-1 で「手動移動を案内」と明記、自動化は別 follow-up |
...
|------|------|
| 推奨モード | incremental |
| 判断根拠 | 単一 md ファイル新設、コード変更なし、テスト影響なし、他施策と競合しない |
| 競合リスク | なし |
| 想定実装時間 | 短（Write 1 回 + grep 検証） |

---

## 実装フロー（autopilot 用）

1. ディレクトリ作成: `mkdir -p .claude/skills/zenigame-fx-plan-and-design`
2. SKILL.md 書き出し: 上記「変更後コード」内の markdown ブロック全体を `Write` ツールで `.claude/skills/zenigame-fx-plan-and-design/SKILL.md` に保存
3. 検証 grep: `grep -cE "..." SKILL.md` → 0 件
4. 必須セクション確認: 上記テスト計画のリスト
5. `.claude/skills/zenigame-fx-codex-review/SKILL.md` の §session_label 命名規則に `todo-selection` を追加（波及変更）
6. Codex impl-review（gpt-5.3-codex high、label: impl-review）APPROVED まで最大 1 round
7. AGENTS.md の skill 説明セクション更新（必要に応じて）
8. コミット
