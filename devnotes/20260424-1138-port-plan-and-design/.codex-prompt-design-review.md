...
【監査・レビュー時の必須チェック — discipline C1-C9】Design-first / 「X が無い = バグ」禁止 / Collider bias / 前提明示 / 並列 sub-agent framing / Fact-Interpretation 分離 / Sample size / INCONCLUSIVE 第一級 / Falsification-first。

---

あなたは経験豊富なクオンツ・システムアーキテクトです。zenigame-fx Alpha Factory の skill port 詳細設計をレビューしてください。
...
- 未移植の hook（zenigame-fx-post-run-review / zenigame-fx-alpha-sieve / zenigame-fx-set-focus）はコメント化 + no-op + 「整備後に接続」明記
- ルックアヘッドバイアス / パフォーマンスチェックは該当なし（md only）

【レビュー観点】
1. 各セクションの整合性（呼び出し契約 / 引数 / 入出力 / 状態ファイル更新責務）
2. 既存 zenigame-fx skill との整合性（codex-review / codex-vscode / analyze-run / run-report / alpha-design / todo-add / improve-cycle）
3. テスト計画（grep 検証 + 必須セクション存在確認）の網羅性
4. 副作用・後退リスク（codex-review への波及変更が妥当か）
5. 考慮漏れ（autopilot / improve-cycle 接続、Conditional promote、TODO closed.md 読み取り経路）
6. 波及変更の網羅性（AGENTS.md / codex-review SKILL.md / improve-cycle / autopilot）
7. **継承先 zenigame-fx-codex-review の必須節名固定**が SSoT として一貫しているか
8. **Codex プロンプト内に「使命・禁止事項」が重複していないか**（zenigame-fx-codex-review が自動挿入する前提）
9. **合議ループ収束ルール**がループ全体で一貫しているか
10. **未接続 hook の no-op 契約**が明確か
11. **スクリプト呼び出しコマンド**（uv run python scripts/alpha_factory/todo_manager.py 等）が実在 subcommand と一致しているか
12. **状態ファイル更新責務**で書き込み key と読み取り key の境界が improve-cycle と矛盾しないか
13. **mission-facing 必須記入欄**（target_metric / failure_mode / causal_path / falsification / success_criterion）が一貫して全 Phase で要求されているか
...
# 陳腐化時
uv run python scripts/alpha_factory/todo_manager.py obsolete \
  --id "{todo_id}" --closed-at "$(TZ=Asia/Tokyo date '+%Y-%m-%d %H:%M')" --run-id "manual"
```
