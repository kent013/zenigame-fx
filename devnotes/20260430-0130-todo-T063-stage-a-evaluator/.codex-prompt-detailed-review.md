...
# T063 詳細設計 — レビュー依頼 (Round 1)

## 役割

zenigame-fx Alpha Factory selection cascade port (synthesis 18 章 + Codex Round 11-20 確定) における **T063 (Stage A evaluator) 詳細設計** のレビュアです。 概念設計は Round 3 で APPROVED 済 (Round 3 [Warning] 2 件 / [Suggestion] 3 件は本詳細設計で吸収)。

## 文脈

- M1 (T058-T060) 設計完了、 M2 進行中 (T061 / T062 概念+詳細 APPROVED + TODO 登録、 T063 概念 APPROVED で詳細レビュー中)
- T063 は Stage A evaluator (orchestrator + ranking、 pure function 化、 controller class 廃止)
- 上位文書: synthesis.md § 5.1 / § 5.5 / § 8.7
- 概念設計 APPROVED: `devnotes/20260430-0130-todo-T063-stage-a-evaluator/conceptual-design.md`

## レビュー観点 (Falsification-first)

1. 数式 synthesis § 5.1 / § 8.7 厳密準拠
2. derive_stage_a_thresholds の Decision 1, 2, 4 実装と前提ガード
3. q_force 動的計算 (base + divergence) の式実装
4. update_divergence_state pure function の semantics
5. immutable state pass-through API の一貫性
6. evaluate_generation の dependency injection (evaluate_fn) の明確性
7. default-deny 契約の docstring 強度
8. テスト計画の網羅性 (4 代表ケース + saturation/recovery + 境界値 + state immutability)
9. C2 parallel-path 5 段階 grep DoD
10. Phase 2 申し送り 8 箇所 + default-deny 検証 test の十分性
11. 学術引用の妥当性
12. その他、 設計の弱点・盲点
...
def compute_q_force_with_divergence(
    ...
    Args:
        ...
        divergence_offset_steps: state.divergence_offset_steps (unsigned >= 0)
...
def evaluate_generation(
    ...
    *,
    ...
    Args:
        ...
        bucket_validator: T072 で実装する provider、 None なら skip (T061 経由で消費)
...
#### update_divergence_state
- `test_update_divergence_state_corr_below_0_5_increments_steps`
- `test_update_divergence_state_corr_at_or_above_0_5_decrements_steps`
...
#### select_top_q_force_indices (Round 2 [Critical])
...
- feasible_ratio_ema は標準的 EWMA、 specific 論文引用なし
