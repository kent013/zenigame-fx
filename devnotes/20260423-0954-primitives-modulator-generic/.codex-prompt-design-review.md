...
先人の知恵を探せ。学術論文・実務文献の引用を歓迎する（著者・年・タイトルを明記）。
...
【C1-C9 discipline】前版のレビューと同じ。design-first / 「X が無い = バグ」禁止 / Fact / Interpretation 分離 / INCONCLUSIVE 許容 / Falsification-first。

---

# System — T012 詳細設計レビュー

あなたは zenigame-fx Alpha Factory の技術設計レビュアーです。以下の詳細設計ドキュメントをレビューしてください。

## 対象

- `devnotes/20260423-0954-primitives-modulator-generic/detailed-design.md`（下に全文添付）
- 概念設計（参考）: `devnotes/20260423-0954-primitives-modulator-generic/conceptual-design.md`
- 関連既存コード: `src/alpha_factory/primitives/_base.py`, `directional_generic.py`, `_registry.py`, `_indicators.py`, `evaluator.py`, `src/events/calendar.py`

## 観点

1. **API 後方互換**: `EvaluationContext` への `event_snapshot / vix_snapshot` 追加が既存 T011 テスト・呼び出しを壊さないか
2. **look-ahead bias**: M5 の bisect_left strict less than、M4 の actual 不参照、M2 の bar_time 自参照、M1/M6 の Wilder 過去参照、M3 の自 bar spread、すべて正当か
3. **registry 整合**: `ensure_registered` が F1-F14 と M1-M6 の両方を bootstrap、`list_by_category("MODULATOR")` == 6 になるか
4. **数値安定**: sigmoid overflow / 0 除算 / NaN 伝播が適切か
5. **warning 発出**: `_M4_WARN_FLAG` / `_M5_WARN_FLAG` の module-level flag が test isolation で問題にならないか
6. **ParamSpec range の妥当性**: 特に M1 threshold（pair ごとに違う price-unit）、M2 soft_edge_min, M3 threshold_bps, M4 window_min/min_impact, M5 threshold（VIX 相場感）、M6 theta
7. **テスト網羅性**: 6 primitive + look-ahead property + 後方互換 F1-F14、mock snapshot の正当性
8. **F6 の `_SESSION_RANGES_UTC` 移動**: import 循環が起きないか、既存 F6 テストが壊れないか
9. **Definition of Done の達成可能性**

## 出力

APPROVED / NEEDS_REVISION verdict。NEEDS_REVISION の場合 Must-fix / Should-consider を分ける。

---

# 詳細設計 (detailed-design.md 全文)
...
- T011 の既存テストは `event_snapshot / vix_snapshot` を指定せず構築するので壊れない（frozen dataclass への default 値付きフィールド追加は backward-compatible）。
...
def _m1_compute_all(ctx: EvaluationContext) -> np.ndarray:
    ...
    raw = direction * (a - threshold) / (scale + _EPS)
    out = sigmoid(raw)
...
M1_SPEC = PrimitiveSpec(
    ...
    param_schema=(
        ...
        ParamSpec(name="threshold", low=0.0001, high=0.02, is_int=False, default=0.005),  # price-unit ATR
        ParamSpec(name="scale", low=0.00001, high=0.01, is_int=False, default=0.002),
...
**閾値 range の根拠**: EURUSD 時間足 ATR は概ね 0.0005 - 0.005 の範囲、USDJPY は 0.05 - 0.5 の
範囲。generic primitive として `threshold` は price-unit だが、pair ごとに全く違うため GA param
range を広めに取る（[0.0001, 0.02]）。pair_specific な正規化は後続で ADR 相対 option に拡張可能。
...
def _m2_compute_all(ctx: EvaluationContext) -> np.ndarray:
    ...
    for i in range(length):
        ...
        if soft_edge_min <= 0:
            ...
            # open edge: sigmoid((minutes - lo_min) / soft_edge_min)
            # close edge: sigmoid((hi_min - minutes) / soft_edge_min)
            # 重なり許容（最小をとる）
            open_gate = sigmoid((minutes - lo_min) / soft_edge_min)
            close_gate = sigmoid((hi_min - minutes) / soft_edge_min)
            out[i] = float(open_gate * close_gate)  # 0/1 の blend
    return out
...
```python
_M4_WARN_FLAG = {"warned": False}  # module-level
...
def _m4_compute_all(ctx: EvaluationContext) -> np.ndarray:
    ...
    if ctx.event_snapshot is None:
        if not _M4_WARN_FLAG["warned"]:
            warnings.warn(
                "M4 EconomicEventGate: event_snapshot is None; returning 1.0 safe default. "
                "Production path must provide EconomicEventSnapshot.",
                RuntimeWarning,
                stacklevel=2,
            )
            _M4_WARN_FLAG["warned"] = True
        return np.ones(length, dtype=np.float64)

    calendar = ctx.event_snapshot.calendar
    # bar ごとに「その bar に最も近い high-impact event までの |Δt| 分」を計算
    # O(N_bars * N_events) が MVP。後続最適化可（sort + window）。
    pair = ctx.pair
    try:
    ...
    relevant_events = [
        ...
        if e.impact >= min_impact and e.currency in (base, quote)
    ]
    ...
    for i in range(length):
        ...
        # |Δt| < window → suppress 寄り（0 に近づく）
        # raw = (window - |Δt|) / scale → sigmoid → [0,1], positive のとき 1 寄り
        # gate = 1 - sigmoid(raw) → event 内で 0 寄り、外で 1 寄り
...
look-ahead: `EconomicEvent.actual` を参照しない。`event.event_time` のみ使う。MVP では
calendar 全量を as_of=+∞ で使う仮定を docstring に明記。

### 3.5 M5 VIXRegimeGate

```python
_M5_WARN_FLAG = {"warned": False}
...
def _m5_compute_all(ctx: EvaluationContext) -> np.ndarray:
    ...
    if ctx.vix_snapshot is None or not ctx.vix_snapshot.observations:
        if not _M5_WARN_FLAG["warned"]:
            warnings.warn(
                "M5 VIXRegimeGate: vix_snapshot missing; returning 0.5 safe default. "
                "Production path must provide VixSeriesSnapshot.",
                RuntimeWarning,
                stacklevel=2,
            )
            _M5_WARN_FLAG["warned"] = True
...
|----|------|----------|------|----------------|-----------|
| M1 | ATRRegimeGate | MODULATOR | `sigmoid(direction * (ATR(n) - threshold) / scale)` | n[7,28], threshold[0.0001,0.02], scale[1e-5,0.01], prefer_high∈{0,1} | なし |
