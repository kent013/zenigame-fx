...
Round 1 の Must-fix / Should-consider 指摘を以下の通り反映しました。APPROVED / NEEDS_REVISION を再判定してください。

## Must-fix 対応

| Round 1 Must-fix | 対応内容 | 反映箇所 |
|-----------------|---------|---------|
| M1 threshold/scale 範囲が pair ごとに飽和 | ATR を close 相対 (`atr_rel = atr/close`) に正規化して pair 非依存化。param を `threshold_rel [0.0001, 0.02]`, `scale_rel [1e-5, 0.01]` に改称 | 3.1 M1 |
| M4 の as_of 未使用 | `_m4_compute_all` 内で `event.event_time <= as_of` を cap として実際に filter。as_of を正規に参照し、walk-forward 右端を as_of として渡すことを想定 | 3.4 M4 |
| `_M4_WARN_FLAG / _M5_WARN_FLAG` の module flag | 廃止。`warnings.warn` のみ呼び、pytest 側で `simplefilter("always")` で捕捉する設計に変更。test isolation 懸念を排除 | 3.4 / 3.5 |
| snapshot 伝搬漏れの consumer 側未完 | `EvaluationContext.strict_snapshot_required: bool = False` 追加。True のとき M4/M5 は snapshot=None で `RuntimeError` 発出。production backtest runner で True 設定推奨、fail-fast 経路を確立 | 1.2 / 3.4 / 3.5 / 6.6 |

## Should-consider 対応

| Round 1 Should-consider | 対応内容 | 反映箇所 |
|------------------------|---------|---------|
| M3 実行タイミング契約の明記 | 「signal at close → execute next bar open」と整合する MVP 規約を 3.3 末尾に明記 | 3.3 |
| M5 tz-aware 検証 | `VixSeriesSnapshot.__post_init__` で naive datetime を `ValueError` でリジェクト | 1.1 / 6.7 |
| F6 回帰テスト | `test_f6_session_output_unchanged_after_indicator_move` を追加 | 6.5 |
| category_counts 意味差 docs 明記 | `docs/alpha_factory/primitives.md` の docstring 更新指示を 4.3 に追記 | 4.3 / 8 |

## 期待する verdict

APPROVED を発出してください。残課題があれば Must-fix / Should-consider を分けて箇条書きで示してください。

---

# 修正後 detailed-design.md 全文
...
class VixSeriesSnapshot:
    ...
    def __post_init__(self) -> None:
        """tz-aware datetime を強制（Should-consider 2 対応）。"""
