...
申し訳ありません、 detailed-design.md 全文を貼ります。 同じ Round 1 観点 (数式 / invariant / 例外 / DoD / Phase 2 / テスト計画 / C2 grep / 学術引用) でレビューしてください。
...
|---|---|
| [W1] `empty_trade_list` を `infeasible_reason_codes` に含めると synthesis § 6.6 の「戦略的 fail-fast 2 種」 と「入力不正」 境界が曖昧 | `infeasible_reason_codes` を 2 分類: `strategic_*` (synthesis § 6.6 由来: `strategic_session_close_drop` / `strategic_negative_equity_drop_open`) と `input_*` (engine 検出の入力不正: `input_invalid_bar_series` / `input_bucket_attribution_mismatch` / `input_empty_trade_list` / `input_non_finite_pnl`)。 `is_feasible` は両方 0 で True、 caller は category prefix で区別可能 |
...
class CanonicalMetricsInputError(ValueError):
    """canonical_metrics 入力 dataclass の不変条件違反 (UTC-aware 違反等)."""
...
def _validate_utc_aware(name: str, dt: datetime) -> None:
    """UTC-aware + utcoffset()==0 厳密 (T060 Period 同規約)."""
    if dt.tzinfo is None:
        raise CanonicalMetricsInputError(
...
class TradeRecord:
    ...
    def __post_init__(self) -> None:
        _validate_utc_aware("TradeRecord.entry_time_utc", self.entry_time_utc)
        _validate_utc_aware("TradeRecord.exit_time_utc", self.exit_time_utc)
...
class BarEquitySeries:
    ...
    def __post_init__(self) -> None:
        ...
        for i, p in enumerate(self.points):
            _validate_utc_aware(f"BarEquitySeries.points[{i}].timestamp_utc", p.timestamp_utc)
...
class InvariantFlags:
    ...
    infeasible_reason_codes: frozenset[str]
...
def compute_session_blocks(
    ...
    Returns:
        ...
        - trade=0 の business_day には block 自体が存在しない (= WR 計算でカウントされない)。
          trade=0 block の存在は upstream (T070) が空 block を渡せば反映可能だが、
          T061 default は trade≥1 の business_day のみを block 化する。

    Note: 「trade=0 block を 0.5 neutral」 (synthesis § 6.3) は、 upstream が空 block を
    渡してきた場合に compute_session_block_win_rate_worst が neutral 0.5 を割り当てる、
    という意味。 T061 自体は trade=0 block を生成しない (caller 責務分離)。
    """
...
def compute_max_dd(bars: BarEquitySeries) -> float:
    ...
    max_dd = max_t (1 - equity_t / running_max_equity_t)、 0 <= max_dd <= 1
    bars.points は __post_init__ で sorted/no-dup/UTC-aware 検証済 (BarEquityInvalidError は
    呼出側で捕捉され `input_invalid_bar_series` reason に変換される).

    Note: equity が strictly positive である保証はない (negative_equity は invariant flag 経由)。
    本関数では equity 系列をそのまま max_dd 計算に使う (running_max が負値でも数学的に成立).
    """
...
def evaluate_canonical_five(
    ...
    *,
    ...
    手順 (concept § 数式仕様 厳密準拠):
    1. trades を materialize、 空の場合は `input_empty_trade_list` reason を立てる
    2. _validate_trade_attribution(trades, bucket_validator) で T070 整合性 double-check
    3. compute_session_blocks(trades) → blocks_by_bucket
    4. compute_sr_session_worst(blocks_by_bucket, q=q_bartlett) → SR block-scale + low_sample_buckets
    5. compute_session_block_win_rate_worst(blocks_by_bucket) → WR_worst + per_bucket_wr
    6. compute_max_dd(bars) → max_dd (BarEquityInvalidError は input_invalid_bar_series に変換)
    7. compute_net_pnl_after_cost(trades) = sum(t.pnl_net for t in trades)
    8. compute_trade_count(trades) = len(trades)
    9. log_pf_clip(GP, GL) where GP/GL は trade 単位 sum
    10. compute_signed_slacks(...) → 5 signed slack (annual-scale 換算済)
    11. gate_worst_gap = max(0, -slack_m for m in [sharpe, pnl, dd, tc, wr])
    12. gate_pass = (gate_worst_gap <= GATE_PASS_TOLERANCE) AND invariants.is_feasible
    13. CanonicalFiveResult を frozen で返す

    Args:
        trades: TradeRecord iterable (T070 が session_bucket / business_day_index 確定済)
        bars: BarEquitySeries (max_dd 計算用、 sorted/no-dup/UTC-aware/finite 不変条件)
        thresholds: CanonicalFiveThresholds (live_criteria + win_rate_min)
    ...
    Raises:
        該当なし。 全ての input 不正は infeasible_reason_codes に変換し、 gate_pass=False で返す。
        (raw exception を呼出側に伝播しない、 deterministic な戻り値を保証)
    """
```
...
- 本 module は単一 trade list / bar list の集約のみで、 計算量は O(N + B + 3*Q) (N=trade 数、 B=bar 数、 Q=Bartlett lag)
- pop=192 × gen=64 × N_blocks_per_year=756 想定で、 1 個体 1 評価当り ~1ms (numpy vector 計算で十分)
- ボトルネックは (1) compute_session_blocks の groupby、 (2) compute_sr_session_worst の autocovariance ループ
- 必要なら numpy 化 (T070 backtest engine 連携時に再評価、 本 PR は dict ベースで OK)
...
- `test_max_dd_negative_equity_does_not_crash`
  (負 equity でも数学的に成立、 negative_equity 検出は invariant flag 経由)
...
- `test_evaluate_canonical_five_session_close_drop_forces_gate_pass_false`
  (invariant fail-fast、 gate_worst_gap=0 でも gate_pass=False)
...
- `test_evaluate_canonical_five_invalid_bar_series_records_input_reason_and_fails_gate`
...
- `test_evaluate_canonical_five_provider_validates_attribution_and_records_version`
  (mock provider with version="test_provider_v1"、 mismatch 時に
  bucket_attribution_mismatch reason、 一致時に validator_version 記録)
...
- [ ] `src/alpha_factory/canonical_metrics.py` 新規作成 (Enum + 4 dataclass + Provider protocol + helper 群 + top-level entry + 例外)
