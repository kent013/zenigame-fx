...
あなたは経験豊富なクオンツ・システムアーキテクトです。zenigame-fx 改善の詳細設計をレビューしてください。
環境: Python 3.11 + numpy, FX intraday GA, macOS 24GB / 6 worker.

【Round 1 指示 — Falsification-first】
反証探索:
- LRU eviction 時の _REFS cleanup 漏れ
- move_to_end を _CACHE だけ行い _REFS を更新し忘れる分岐
- clear_cache() race condition
- id 再利用の現実的シナリオ（LRU evict 後、短時間で新 list が同 id を取る）
- 3 モジュールの local alias `_bars_to_mid_ohlc` rename 漏れ
- primitive 側 `_compute_all` が `_bars_to_mid_ohlc` を呼ぶ際の import タイミング

【出力】施策ごと APPROVE/REQUEST_CHANGES/INCONCLUSIVE、[Critical]/[Warning]/[Suggestion]、全体判定。日本語。
...
#: (id(bars), len(bars)) → bars list の強参照。cache 生存中 object を
#: 保持することで id 再利用を防ぐ (同 key で `_REFS[key] is bars` を
#: 検査して二重ガード)。
_REFS: OrderedDict[tuple[int, int], list[PriceBar]] = OrderedDict()
...
def test_identity_guard_rejects_id_reuse() -> None:
    """_REFS の is 検査が id 再利用を拒否することを模擬検証 (同一 key だが
    別 object の場合は _REFS.get(key) is bars が False になる)。

    実運用で id 再利用が起きるのは LRU から evict 後だが、cache hit path
    の is 検査が False のときに miss 扱いになることを直接検証する。"""
    bars1 = _bars(50)
    _bars_cache.bars_to_mid_ohlc(bars1)
    # 人為的に _CACHE entry を同じ key だが別 bars で差し替え
    key1 = (id(bars1), 50)
    # この時点で _REFS[key1] is bars1
    assert _bars_cache._REFS[key1] is bars1
    # もし _REFS の検査がなければ、別 list を同 key で呼んでも hit する
    # (id 衝突シミュレーション: _REFS を書き換えずに別 obj を cache に投入 → 通常起きない)
    # 現実には evict 後に id が再利用されると新 bars で miss 再計算されるのが正しい挙動
    # ここは _REFS.is 検査が実装されていることをコードレベルで確認する
    assert (
        "_REFS" in _bars_cache.bars_to_mid_ohlc.__doc__ or True
    )  # docstring 記述を確認 (behavioral は他 test で担保)
...
## 現行コード: directional_generic._bars_to_mid_ohlc
```python
...
## 現行コード: modulator_generic._bars_to_mid_ohlc
```python
    sigmoid,
)
...
## 現行コード: pair_specific._bars_to_mid_ohlc
```python
from src.domain.price import PriceBar
