# Conceptual Design Review — run-ga-full-rewrite (T018)

あなたは zenigame-fx Alpha Factory のアーキテクト役です。以下の概念設計をレビューしてください。

## 背景

- リポジトリは zenigame-fx（FX 向け Alpha Factory）。Clause ベース Genome + Stage A/B/C + archive + cross-pair shadow + swim lane manager はすでに実装済み。
- 本 TODO (T018) は `scripts/alpha_factory/run_ga.py` を全面改修し、上記基盤を統合することが目的。
- 実装パス・設定ファイル:
  - `src/alpha_factory/{stage_gate,archive,cross_pair,swim_lane,statistics,primitives/}`
  - `src/ga/{runner,operators,random_gen,complexity,fitness}.py`
  - `src/dsl/{genome,strategy,serialize}.py`
  - `src/backtest/engine.py`
  - `config/alpha_factory/default.yaml`

## レビュー観点

1. フローの過不足（漏れている統合 / 冗長な処理）
2. fitness の lexicographic 合成（Stage A fitness_pen + 通過 boost）が GA 選択と整合しているか、より良い案があれば提案
3. Bars の 3 区間分割（Stage A=60d / B=18m / C=holdout 60d）の意味論変更が既存 config や downstream tooling (analyze_run / generate_run_report) と整合するか
4. `src/alpha_factory/config.py` 新設の是非。`stage_gate.StageGateConfig` / `cross_pair.CrossPairConfig` を再エクスポートする設計が OK か
5. LaneManager との責務分離（population 生成を run_ga に、Stage 評価を LaneManager に）が妥当か
6. Phase 2 で cross-pair shadow を単一 instrument でも意味のある形で動かせるか
7. 出力契約（summary.json など）の後方互換性

## 期待する出力形式

- **判定**: APPROVED / NEEDS_REVISION のいずれか
- **コメント**: 箇条書き 5〜10 点
- NEEDS_REVISION の場合は具体的な修正指示を提示

---

## 概念設計

以下は `devnotes/20260423-2324-run-ga-full-rewrite/conceptual-design.md` の全文です。
