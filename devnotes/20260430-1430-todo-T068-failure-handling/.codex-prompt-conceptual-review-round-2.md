# T068 概念設計 — Round 2 (Round 1 修正反映、 再レビュー依頼)

Round 1 で `CHANGES_REQUESTED`、 [Critical] 3 + [Warning] 5 + [Suggestion] 5 を全て反映済。

## Round 1 → Round 2 対応マトリクス

| Round 1 指摘 | Round 2 修正対応 | 該当 § |
|---|---|---|
| [C1] n_failed = len(records) で同 genome 複数 stage 失敗時誤判定 | FailureSummary を **n_failure_records / n_failed_genomes / eligible_individuals / all_failed** 4 field 分離。 abort は n_failed_genomes ベース | § 5.2 / § 9.1 |
| [C2] 全 fail 判定の分母が stage-local でない | aggregate_failures を `(records, *, stage, eligible_count)` で **stage-local** に。 RunFailureSummary は per-stage の集約 monitor 用のみ | § 5.2 / § 9.1 |
| [C3] degraded result downstream 伝搬契約未閉包 | **EvaluationOutcome[T] dataclass** で `(result, failure_record, should_skip_downstream)` 統一。 caller は should_skip_downstream=True 個体を T065 入力から除外 (R1 案 A 概念で確定) | § 6.1 / § 11.2 |
| [W1] contract_violation の分類規約未確定 | wrapper で `except ValueError → contract_violation` / `except Exception → exception_raised` の 2 段 catch を明文化 | § 6.1 |
| [W2] validate docstring vs candidates の trade_count 不整合 | `trade_count` (int) は finite check 不要として candidates から削除 | § 8.1 |
| [W3] stage 型が wrapper str / FailureRecord Literal 不整合 | wrapper API も `StageType = Literal[...]` で統一 | § 5.1 / § 6.1 |
| [W4] KeyboardInterrupt 透過のみ、 SystemExit 不明 | docstring に「`except Exception` は SystemExit / KeyboardInterrupt / GeneratorExit を捕捉しない (Python 標準)」 明記 | § 6.1 |
| [W5] failure_rate total=0 意味論曖昧 | eligible_count=0 で `failure_rate=0.0`、 `all_failed=False` を docstring 明記 | § 9.1 |
| [S1] FailureSummary 4 field 分離 | 採用 (Critical 1 と統合) | — |
| [S2] FailureRecord に exception_fingerprint | `exception_fingerprint: str` field 追加 (例: "ValueError@canonical_five:slack_pnl_nan"、 dedup 用) | § 5.1 |
| [S3] validate_finite_bc_result の nested 検査範囲 | top-level + StageBResult / StageCLiteResult / StageCResult sub-result の主要 float field を candidates に含める。 詳細設計で完全リスト | § 8.3 |
| [S4] R2 sentinel invariant 1 行固定 | § 8.4 で `is_feasible=True ⟺ math.isfinite(mission_signed_margin) AND >=0 AND mission_inf_gap==0 AND constraint_violation==0` を明文化 | § 8.4 |
| [S5] zenigame 踏襲度 stage-local 分母定義 | Critical 2 と統合 (stage 単位 aggregate) | — |

## Round 2 着目点 (確認のみ)

- FailureSummary が stage-local + 4 field 分離 (n_failure_records / n_failed_genomes / eligible_individuals / all_failed)
- aggregate_failures が `(records, *, stage, eligible_count)` で stage 単位
- EvaluationOutcome[T] で should_skip_downstream を caller に伝達 (R1 案 A 確定)
- exception_fingerprint で dedup
- sentinel 許容 invariant 1 行 (is_feasible truth table の固定 anchor)
...
- APPROVED (Round 1 [Critical] 3 / [Warning] 5 / [Suggestion] 5 全反映、 stage-local SSOT 確定)
- CHANGES_REQUESTED
- BLOCKED
...
class FailureRecord:
    ...
    exception_fingerprint: str     # dedup 用 (例: "ValueError@canonical_five:slack_pnl_nan"、 Round 1 [S2])
```
