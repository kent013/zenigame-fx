# T068 概念設計 — Round 3 (Round 2 修正反映、 再レビュー依頼)

Round 2 で `CHANGES_REQUESTED`、 [Critical] 3 + [Warning] 4 + [Suggestion] 3 を全て反映済。

## Round 2 → Round 3 対応マトリクス

| Round 2 指摘 | Round 3 修正対応 | 該当 § |
|---|---|---|
| [C1] failure_reason に state_inconsistency なし (§ 8.4 で導入したが § 5.1 未追加) | FailureRecord.failure_reason Literal に `"state_inconsistency"` 追加 (4 種類) | § 5.1 |
| [C2] § 4.1 / § 4.2 が旧 API のまま | § 4.1 を EvaluationOutcome 戻り値 + ValueError/Exception 2 段 catch + state invariant check に書直し。 § 4.2 を stage-local + eligible_count に書直し | § 4.1 / § 4.2 |
| [C3] § 10.4 と § 7.2 で finite cap 言及残留 | § 10.4 を「`should_skip_downstream=True` 個体は caller が T065/T066/T067 から除外、 finite cap 不要」 1 文確定 (Round 2 [Suggestion 2] 採用) | § 10.4 |
| [W1] § 3.1 dataclass 一覧 EvaluatorFailure (旧名) | § 3.1 を EvaluationOutcome に統一、 EvaluatorFailure 廃止明記 + state invariant validate API 列挙 | § 3.1 |
| [W2] § 11.1 FailureSummary per-Run 記述 | § 11.1 dataclass 一覧を stage-local FailureSummary + RunFailureSummary + EvaluationOutcome に同期 | § 11.1 |
| [W3] テスト計画旧仕様残存 | `test_aggregate_failures_groups_by_stage` → `_filters_records_by_stage`、 `_raises_on_n_failed_exceeding_total` → `_n_failed_genomes_exceeding_eligible_count`、 `_failures_by_stage_is_mapping_proxy` → `_failures_by_reason_is_mapping_proxy` | § 12 |
| [W4] exception_fingerprint dataclass `str` vs 表 `str | None` | dataclass を `str | None` 統一 (reason によっては fingerprint 構築不能、 None 許容) | § 5.1 |
| [Suggestion 1] § 4 を新 API 全面同期 | 採用 (Critical 2 と統合) | § 4 |
| [Suggestion 2] § 10.4 を最終契約 1 文化 | 採用 (Critical 3 と統合) | § 10.4 |
| [Suggestion 3] § 16 残論点 5 件を現状更新 | § 16 「残論点 3 件 (R3/R4/R5)、 R1/R2 概念で確定」 に更新 | § 16 |

## Round 3 着目点 (確認のみ)

- failure_reason 4 種類 (exception_raised / contract_violation / non_finite_detected / state_inconsistency) で完備
- § 4.1 / § 4.2 が § 11.2 SSOT と完全同期
- § 10.4 の最終契約 1 文 (should_skip_downstream で除外、 finite cap 不要)
- EvaluatorFailure 旧名完全削除、 EvaluationOutcome 統一
- テスト名の旧仕様残存解消

## Round 3 verdict 期待

- APPROVED (Round 2 [Critical] 3 / [Warning] 4 / [Suggestion] 3 全反映、 SSOT 完全閉鎖)
- CHANGES_REQUESTED
- BLOCKED
