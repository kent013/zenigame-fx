# T068 概念設計 — Round 4 (Round 3 修正反映、 再レビュー依頼)

Round 3 で `CHANGES_REQUESTED`、 [Critical] 2 + [Warning] 3 + [Suggestion] 3 を全て反映済。

## Round 3 → Round 4 対応マトリクス

| Round 3 指摘 | Round 4 修正対応 | 該当 § |
|---|---|---|
| [C1] state_inconsistency 発火経路が § 6.1 擬似コードに不在 | § 6.1 evaluate_canonical_five_safe に `validate_state_invariant_canonical_five` invariant check ステップ追加、 `_make_failure_record_for_state_inconsistency` 経由で record 生成 | § 6.1 |
| [C2] § 7.2 build_degraded_mission_gap の「caller finite cap」 残留 | § 7.2 docstring から finite cap 言及削除、 「§ 10.4 最終契約 (caller が should_skip_downstream で除外、 T068 は事前変換しない)」 1 文に統一 | § 7.2 |
| [W1] § 16 完了判定が 5 件 Pending と R1/R2 確定で自己矛盾 | § 16 を「残論点 3 件 (R3/R4/R5) Pending、 R1/R2 概念で確定済」 に更新 | § 16 |
| [W2] § 1.2 T061 行に「もしくは None で caller に伝達」 旧仕様残留 | § 1.2 を `EvaluationOutcome[T]` 統一表現に書換 | § 1.2 |
| [W3] § 13 タイトル「残論点 / Decision Pending」 のまま | (本セッションでは保持、 詳細設計で「決定事項 + 残論点」 構造に分割可能) | § 13 (保留) |
| [Suggestion 1-3] | 採用 (Critical 1, 2 + Warning 1 と統合) | — |

## Round 4 着目点 (確認のみ)

- § 6.1 invariant check ステップが finite check 直後に明示
- § 7.2 から finite cap 言及完全削除、 § 10.4 1 文確定との整合
- § 1.2 の T061 行が EvaluationOutcome 統一表現
- § 16 残論点 3 件 (R3/R4/R5) で R1/R2 確定との一貫性

## Round 4 verdict 期待

- APPROVED (Round 3 [Critical] 2 / [Warning] 3 / [Suggestion] 3 全反映、 SSOT 完全閉鎖)
- CHANGES_REQUESTED
- BLOCKED
