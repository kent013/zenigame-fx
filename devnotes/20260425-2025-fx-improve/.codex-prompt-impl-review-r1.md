あなたは経験豊富な Python コードレビュアーです。zenigame-fx Alpha Factory の T031 (regime-participation-constraint Phase 1) 実装をレビューしてください。
...
# T031 詳細設計の要点
trade_count=0 個体を selection_score の feasibility 要素で淘汰する Phase 1 実装。
- selection_score 4要素 (C,B,A,fp) → 6要素 (feasible, -violation, C, B, A, fp) 化
- fallback: cache 全体が infeasible なら旧 4要素にフォールバック (cache-global 判定、sample 局所判定はしない)
- apply_from_generation で適用世代を制御
- 7 施策: GAFeasibilityConfig dataclass / default.yaml / IndividualCacheEntry 拡張 / _update_cache / summary 出力 / generate_run_report / テスト

# テスト結果
- 全 974 tests pass + 1 skipped
- T031 新規 18 tests + 既存 smoke test 更新 → 全 pass
- ruff: T031 関連ファイル全 clean
- mypy: src/ 全 90 files clean
...
+            (best.get("metrics") or {}).get("trade_count") or 0
+        )
...
# レビュー観点
1. 設計との一致性 (`devnotes/20260425-0937-regime-participation-constraint/detailed-design.md`)
2. 正確性 (ロジック / エッジケース / NaN)
3. 一貫性 (selection_key / fallback の 3 箇所 tournament/elite/select_best 同一規則)
4. テスト網羅性 (各施策にテストあるか、特に fallback 規則一貫性)
5. パフォーマンス (cache.values() の繰り返し呼び出し)
6. ruff / mypy 準拠
7. 禁止事項違反なし

# 出力形式
- ファイルごとに [Critical] [Warning] [Suggestion] を明示
