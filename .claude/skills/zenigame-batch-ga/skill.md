---
name: zenigame-batch-ga
description: GA-OnlyバッチN回連続実行（分析BG、クロスラン統計）
...
> 「2RUN走らせて」「N回まわして」等の一般的なRUN指示では `/zenigame-improve-cycle` を使うこと。

コード凍結状態でGAをN回連続実行し、ベースライン分布を確立する。
分析はバックグラウンドで実行し、GA回転速度を最優先する。
...
|------|----------|------|
| `--n` | 10 | 実行回数 |
| `--calibrate` | off | 各Run間にcalibrate-gateを実行 |
| `--label` | なし | バッチのラベル（比較用） |
| `--snapshot-before` | なし | バッチ開始前にスナップショット作成 |
| `--run-args` | 前回Run設定 | GA実行引数（`/zenigame-run-alpha-factory` に渡す） |
| `--resume` | なし | 中断バッチの再開（batch_id指定） |
...
`--snapshot-before` 指定時:
```
/zenigame-snapshot create {snapshot_name}
```

**0-3. run_args決定**

`--run-args` が明示指定されていない場合、最新のRunレポートから設定を取得する:
```bash
...
- `--warmstart-auto`（前回Runからの引き継ぎ）

**0-4. batch_state.json書き込み**
...
### Phase 1: 連続GAループ (i = 1..N)

以下をN回繰り返す。

**1-1. batch_state.json更新**
...
```
/zenigame-calibrate-gate {prev_run_id}
```

calibrate前後の `stage_b_ratio` を記録しておく（runs配列に含める）。

**1-3. GA実行**

`/zenigame-run-alpha-factory` と同等の手順でGA実行する。ただし以下をスキップ:
- **Step 7（ゲノムアーカイブ分析）**: 代わりにextract_batch_metrics.py（軽量10秒）を使用
- **Step 8（繰り返しモード）**: batch-ga側で制御するためスキップ

具体的には `/zenigame-run-alpha-factory` のStep 1〜6を実行する:
1. 前提条件確認
2. バックグラウンド実行: `uv run python scripts/trading/run_alpha_factory.py {run_args}`
3. ログ監視ループ（エラー時のみ報告）
4. 完了検出
5. 結果確認（run_id取得）
6. 結果報告（**簡潔に**: C-PASS数、B-PASS数、Best B-Sharpe のみ）

**1-4. 軽量メトリクス抽出**

```bash
uv run python scripts/trading/extract_batch_metrics.py {run_id} \
  --output .cache/alpha_factory/batch/{batch_id}/metrics/{run_id}.json \
  --report .cache/alpha_factory/batch/{batch_id}/reports/{run_id}.md
```

**1-5. レポート生成（`/zenigame-run-report` スキル呼び出し）**

バッチでも正式レポートと同じ品質で生成する。Mission Dashboard等の全セクションが含まれる。

```
/zenigame-run-report --run_id {run_id}
```

**注意**: レポート生成はGA回転のクリティカルパスに入る。レポート生成中に次のRunを開始することはしない（前Runのレポートが正しく生成されることを保証するため）。

**1-6. batch_state.json更新**
...
| シナリオ | 対応 |
|---------|------|
| GA run失敗 | エラーログ記録、`runs[i].status="failed"`、次Runへ進む |
| 連続3回失敗 | **バッチ中断**: ユーザーに報告して停止 |
| 成功率50%未満（4回以上完了後） | **バッチ中断**: ユーザーに報告して停止 |
| メトリクス抽出失敗 | 警告表示、metrics=null、次Runへ進む |
...
```bash
uv run python scripts/trading/compare_batch_runs.py --batch-dir .cache/alpha_factory/batch/{batch_id}
```

**2-2. batch_state.json最終更新**
...
.cache/alpha_factory/batch/{batch_id}/
  ...
  metrics/                  # Per-run メトリクスJSON
    run_YYYYMMDD_HHMMSS.json
  reports/                  # Per-run 軽量Markdownレポート
...
```bash
# Step 1: ベースライン10回
/zenigame-batch-ga --n 10 --calibrate --label baseline --snapshot-before pre-experiment

# Step 2: ユーザーがコード変更を実施（例: T090/T104/T124を実装・コミット）

# Step 3: トリートメント10回
/zenigame-batch-ga --n 10 --calibrate --label treatment

# Step 4: Before/After比較
uv run python scripts/trading/compare_batch_runs.py \
...
/zenigame-snapshot restore pre-experiment
```
...
- **post-run-reviewはバッチ中全スキップ**（GA回転速度最優先）
- **improve-cycleは呼び出さない**（コード凍結が前提）
- `current_run_state.json` は各Run中のみ使用（batch_state.jsonとは別管理）
